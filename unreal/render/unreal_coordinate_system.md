# BEDLAM2 Unreal Coordinate System Information
BEDLAM2 uses `Unreal coordinate system` (same system as in BEDLAM) for specifying the render sequences (`be_seq.csv`) and for storing the camera ground truth intrinsic and extrinsic parameters in JSON (`ground_truth/meta_exr/`) and CSV (`ground_truth/meta_exr_csv/`) formats.
